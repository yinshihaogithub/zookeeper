            ...
            while ((self.getPeerState() == ServerState.LOOKING) && (!stop)) {
                ...
                if (n == null) {
                    ...
                    case LOOKING:
                        ...
                        // 判断接收到 节点epoch大于逻辑时钟，则表示是新的一轮选举
                        if (n.electionEpoch > logicalclock.get()) {
                            // 更新本地的逻辑时钟
                            logicalclock.set(n.electionEpoch);
                            // 清空接收队列
                            recvset.clear();
                            // 检查收的这个消息是否可以胜出，一次比较epoch、zxid、myid
                            // 规则：epoch > zxid > myid
                            ...
                            // 如果收到的消息epoch小于逻辑时钟，则忽略本次消息
                                ...
                                LOG.debug(
                                    ...
                                    Long.toHexString(logicalclock.get()));
                            break;
                            // 如果epoch相同，则继续比较zxid，myid，更新票据，发送消息
                        ...
                        // 判断选择是否结束，默认算法是超过半数server同意
                        if (voteSet.hasAllQuorums()) {

                            // Verify if there is any change in the proposed leader
                            // 一直等到新的notification 到达，直到超时
